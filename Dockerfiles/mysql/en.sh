...
initialize_mysql_database() {
  ...
  if [ ! -d ${MYSQL_DATA_DIR}/mysql ]; then
    ...
    mysql -uroot -e "CREATE USER 'mysql'@'localhost' IDENTIFIED BY 'mysql';"
    mysql -uroot -e "GRANT ALL PRIVILEGES on *.* TO 'mysql'@'localhost' IDENTIFIED BY 'mysql';"

    if [ -n "${DB_REMOTE_ROOT_NAME}" -a -n "${DB_REMOTE_ROOT_HOST}" ]; then
      echo "Creating remote user \"${DB_REMOTE_ROOT_NAME}\" with root privileges..."
      mysql -uroot \
      -e "GRANT ALL PRIVILEGES ON *.* TO '${DB_REMOTE_ROOT_NAME}'@'${DB_REMOTE_ROOT_HOST}' IDENTIFIED BY '${DB_REMOTE_ROOT_PASS}' WITH GRANT OPTION;"
      mysql -uroot \
      -e "GRANT ALL PRIVILEGES ON *.* TO '${DB_REMOTE_ROOT_NAME}'@'localhost' IDENTIFIED BY '${DB_REMOTE_ROOT_PASS}' WITH GRANT OPTION; FLUSH PRIVILEGES;"
    fi
