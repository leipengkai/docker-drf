...
class MessageLogs(Base):
    ...
    platform = Column(String(16))  # "本公司的某个产品"
...
class MysqlClient():
    ...
    def __init__(self, user=MYSQLUSER, password=MYSQLPAWD, host=MYSQLHOST, port=MYSQLPORT, db=MYSQLDB):
    ...
    def test_add_log(self):
        c1 = MessageLogs(target=TESTMOBILE, verification_code='88888', template_code='SMS_166777256', platform="本公司的某个产品")
        self.session.add(c1)
        self.session.commit()
    ...
    def add(self, target, type, code, routing_key, template_code="", platform="本公司的某个产品"):
